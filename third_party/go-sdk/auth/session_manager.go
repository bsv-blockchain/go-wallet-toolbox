...
//
// LOCAL PATCH (go-wallet-toolbox): upstream removed the session and re-added
// it, leaving a window in which concurrent lookups by sessionNonce failed with
// session-not-found. Peers call UpdateSession after every handled message, so
// under concurrent requests on one session the server would randomly reject
// valid requests (unauthenticated responses → client "missing version header
// in response"). Sessions are mutated in place and keyed by their immutable
// SessionNonce, so an idempotent re-Add (map overwrite) records the same state
// with no removal window.
