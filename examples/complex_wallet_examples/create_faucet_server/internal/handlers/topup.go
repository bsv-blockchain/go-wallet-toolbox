...
func NewTopUpHandler(deps methods.FaucetDeps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&req); err != nil || req.Outpoint == "" {
			return c.Status(http.StatusBadRequest).JSON(TopUpResponse{Status: statusError, Message: "outpoint required (format: txid:outputIndex)"})
		}

		// Parse outpoint using transaction helper
		op, err := transaction.OutpointFromString(req.Outpoint)
		if err != nil {
			return c.Status(http.StatusBadRequest).JSON(TopUpResponse{Status: statusError, Message: fmt.Sprintf("invalid outpoint: %v", err)})
		}

		if err := methods.TopUpInternalize(c.Context(), deps, deps.Wallet, op.Txid.String(), op.Index); err != nil {
			return c.Status(http.StatusInternalServerError).JSON(TopUpResponse{Status: statusError, Message: err.Error()})
		}
