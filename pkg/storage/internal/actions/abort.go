...
func (a *abortAction) AbortAction(ctx context.Context, userID int, args *wdk.AbortActionArgs) (*wdk.AbortActionResult, error) {
	...
	err = repo.DBTransaction(ctx, a.transactionsRepo, func(dbtx TransactionsRepo) error {
		...
		if err := dbtx.FailTransactionByID(ctx, txEntity.ID, txEntity.TxID, history.NewBuilder().AbortAction(referenceStr)); err != nil {
			return fmt.Errorf("failed to update transaction status: %w", err)
		}
		return nil
	})
	if err != nil {
		return nil, fmt.Errorf("cannot submit abort action changes to DB: %w", err)
	}
