...
func TestUpsertCertificateForSync_CreateUpdateAndStaleSkip(t *testing.T) {
	...
	// Restore: newer chunk with IsDeleted=false clears deleted_at.
	tRestore := time.Date(2026, 4, 23, 15, 0, 0, 0, time.UTC)
	_, certID3, err := repos.UpsertCertificateForSync(t.Context(), &entity.Certificate{
		CreatedAt:          tOld,
		UpdatedAt:          tRestore,
		UserID:             user.ID,
		Type:               fixtures.TypeField,
	...
	require.NoError(t, err)
	require.Equal(t, certID, certID3)

	var restored models.Certificate
	require.NoError(t, d.DB.Unscoped().First(&restored, certID).Error)
	require.False(t, restored.DeletedAt.Valid)
	require.Equal(t, "restored-subject", restored.Subject)

	rows2, err := repos.FindCertificatesForSync(t.Context(), user.ID)
	require.NoError(t, err)
	require.Len(t, rows2, 1)
	require.False(t, rows2[0].IsDeleted)
}
