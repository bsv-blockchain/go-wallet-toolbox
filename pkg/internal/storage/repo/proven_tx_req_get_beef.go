...
func (p *ProvenTxReqRepo) GetBEEFForTxID(ctx context.Context, txID string, opts ...entity.GetBEEFOption) (*transaction.Beef, error) {
	var err error
...
func (p *ProvenTxReqRepo) GetBEEFForTxIDs(ctx context.Context, txIDs iter.Seq[string], opts ...entity.GetBEEFOption) (*transaction.Beef, error) {
	var err error
...
func (p *ProvenTxReqRepo) recursiveBuildValidBEEF(
	ctx context.Context,
	depth int,
	...
	if options.IsProvenTxReqID(txID) {
	...
	query := p.db.WithContext(ctx).
		...
		Select("bsv_proven_tx_reqs.rawTx, bsv_proven_tx_reqs.inputBeef, bsv_proven_txs.merklePath").
		Joins("LEFT JOIN bsv_proven_txs ON bsv_proven_txs.provenTxId = bsv_proven_tx_reqs.provenTxId")

	if len(options.StatusesToFilterOut) > 0 {
		query = query.Where("bsv_proven_tx_reqs.status NOT IN ? ", options.StatusesToFilterOut)
	}

	err := query.Where("bsv_proven_tx_reqs.txid = ?", txID).First(&res).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		res.RawTx = rawTx
		res.MerklePath = to.If(merklePath != nil, merklePath.Bytes).ElseThen(nil)
		res.InputBeef = inputBeef
	} else if err != nil {
	...
	tx, err := transaction.NewTransactionFromBytes(res.RawTx)
	if err != nil {
	...
	if (len(res.MerklePath) > 0) && !ignoreMerkleProof {
		var merklePath *transaction.MerklePath
		merklePath, err = transaction.NewMerklePathFromBinary(res.MerklePath)
		if err != nil {
	...
	_, err = mergeToBeef.MergeRawTx(res.RawTx, nil)
	if err != nil {
		return fmt.Errorf("failed to merge raw tx (id: %s) into BEEF object: %w", txID, err)
	}

	if len(res.InputBeef) > 0 {
		err = mergeToBeef.MergeBeefBytes(res.InputBeef)
		if err != nil {
