...
func Address[SenderKey CounterpartyPrivateKey, RecipientKey CounterpartyPublicKey](sender SenderKey, keyID KeyID, recipient RecipientKey, opts ...func(*lockOptions)) (*script.Address, error) {
	options := &lockOptions{
		mainNet: true,
	}
	...
	senderKeyDeriver, err := toKeyDeriver(sender)
	if err != nil {
		return nil, fmt.Errorf("failed to create sender key deriver: %w", err)
	}

	recipientIdentityKey, err := toIdentityKey(recipient)
	if err != nil {
		return nil, fmt.Errorf("failed to create recipient identity key: %w", err)
	}
